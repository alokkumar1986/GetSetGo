...
$query="update `verbaltest` set `college`='".$_POST['college']."', `college_shortname`='".$_POST['shortname']."', `yop`='$yop', `start_date`='".$_POST['sdate']."', `end_date`='".$_POST['edate']."', `duration`='".$_POST['duration']."' where id='".$_POST['id']."'";
mysql_query($query);
...
<fieldset>
<legend><font color="#330099" ><strong>Email Writting/Verbal Test Update Setting</strong></font></legend>
...
<input type='hidden' name='id' id='id' value='<?php echo $_GET['id1']; ?>' />  
 <?php if($_GET['action']=='edit'){ 
     $sqlquery=mysql_query("select * from `verbaltest` where id='$_GET[id1]'");
     if($rowqry=mysql_fetch_array($sqlquery)){ 
       $yop1=explode(",",$rowqry['yop']);  
         ?>
<p>
 ...
 while($rs=mysql_fetch_array($qry)){ ?>
 	<option value="<?php echo $rs['COLLEGE']; ?>" <?php if($rowqry['college']==$rs['COLLEGE']){ ?>selected="selected" <?php } ?>><?php echo $rs['COLLEGE_FULLNAME']; ?></option>
 <?php }  ?>
 </select> </p>
 <p><select name="shortname" id="shortname" style="color: grey;">
 <option value="<?php echo $rowqry['college_shortname']; ?>"><?php echo $rowqry['college_shortname']; ?></option>
 </select>
</p>
...
while($result=mysql_fetch_array($r)){
	?>
	<option value="<?php echo $result['COURSE_YOP']; ?>" <?php if(in_array($result['COURSE_YOP'],$yop1)){ ?> selected="selected" <?php } ?>><?php echo $result['COURSE_YOP']; ?></option>
	<?php }  ?>
 </select> Select YOP
</p>
<div id="datetimepicker1" class="input-append date">
   <input data-format="dd/MM/yyyy hh:mm:ss" type="text" name="sdate" Placeholder="Start Date" value="<?php echo $rowqry['start_date']; ?>"/>
...
<p>			
  <div id="datetimepicker1" class="input-append date">
  End Date  <input data-format="dd/MM/yyyy hh:mm:ss" type="text" name="edate" Placeholder="End Date" value="<?php echo $rowqry['end_date']; ?>"/>
    <span class="add-on"  style="padding:3px;height:30px">
      <i data-time-icon="icon-time" data-date-icon="icon-calendar">
      </i>
    </span>
	...
	<p><input name="duration" id="timepicker1" type="text" value="<?php echo $rowqry['duration']; ?>" class="input-medium time" placeholder="Duration(Only Number (ie. 15))" style="width:220px !important"></p>
	<p><input type='submit' class="btn btn-success" id="button1" name='Submit' value='Update' />&nbsp;&nbsp;&nbsp;<!--<a href="?id=view email tests" class="btn btn-danger" style="float: right;margin-left:10px;margin-right:10px;">View All Tests</a>--></p>
