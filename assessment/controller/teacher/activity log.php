...
<?php $countuser=mysql_num_rows($rs=mysql_query("select * from `activitylog` where `activity`='on'"));
...
<table cellpadding="0" cellspacing="0" border="0" class="display" id="example" width="100%">
	...
	<tbody>
	<?php 
	$sql="select * from `activitylog` order by activity asc";
	$rs=mysql_query($sql);
	while($row=mysql_fetch_array($rs)){
		...
		<td><?php if($row['activity']=='on'){ echo "<font color=green size=4> Online</font>"; }if($row['activity']=='off'){ echo "<font color=red size=2>Offline</font>"; } ?></td>
			...
			<td>
		<?php	$sql22="select * from `student_data` where REG_NO='".$row['reg_no']."'";
